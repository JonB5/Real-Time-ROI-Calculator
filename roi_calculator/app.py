...
def calculate():
    try:
        ...
        calculator.purchase_price = float(data.get('purchase_price', 0))
        calculator.down_payment = float(data.get('down_payment', 0))
        calculator.closing_costs = float(data.get('closing_costs', 0))
        calculator.property_taxes = float(data.get('property_taxes', 0))
        calculator.insurance = float(data.get('insurance', 0))
        calculator.monthly_rent = float(data.get('monthly_rent', 0))
        calculator.monthly_expenses = float(data.get('monthly_expenses', 0))
        calculator.interest_rate = float(data.get('interest_rate', 0.04))
        calculator.appreciation_rate = float(data.get('appreciation_rate', 0.03))
        
        # Generate report and additional calculations
        ...
        return jsonify({
            ...
            'calculations': {
                ...
                'net_profit_1yr': calculator.calculate_net_profit(1),
                'net_profit_5yr': calculator.calculate_net_profit(5),
                'roi_1yr': calculator.calculate_roi(1),
